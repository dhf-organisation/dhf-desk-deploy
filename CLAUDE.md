...
| 9 | Diary/Jobs/Reports load whole tables (all notes, all invoices); PostgREST caps responses at 1,000 rows (`fetchAllRows`, `app-main.js:421`, pages around that cap but still loads everything). `crm.html`'s `loadAll()` (~line 2693) has its own independent copy of the same idea, over a different client (`sb.get()` REST wrapper, not supabase-js) — deliberately not shared, since the two pages have no shared JS and no common query-builder shape to wrap; update both if this is ever fixed for real (decision: [issue #73](https://github.com/dhf-organisation/dhf-desk-deploy/issues/73)). Not broken — tech debt/perf, not a bug: nothing needs a lazy/windowed rewrite yet at current data volume | `app-main.js:421`, `crm.html:2693` | Tech debt/Performance |
